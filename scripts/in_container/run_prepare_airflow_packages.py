...
def build_airflow_packages(package_format: str):
    build_command = [sys.executable, "-m", "hatch", "build"]
    ...
    build_process = subprocess.run(build_command, capture_output=True, text=True, cwd=AIRFLOW_SOURCES_ROOT)

    if build_process.returncode != 0:
        console.print("[red]Error building Airflow packages")
        console.print(build_process.stdout)
