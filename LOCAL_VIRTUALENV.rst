...
+=============+=========+=======================+===============================================================+
| default     | virtual | apache-airflow[devel] | Default environment with Python 3.8 for maximum compatibility |
+-------------+---------+-----------------------+---------------------------------------------------------------+
...
virtualenv for maximum compatibility. The default extra with which the environment is create is "devel", which
should be enough to develop and test basic airflow core tests. You can create the default environment with:
...
.. code:: bash
    ...
    hatch -e airflow-310 run -- pip install ".[google]"
...
.. code:: bash
    ...
    pip install -e ".[apache-beam,dbt-cloud]"
...
``pip install -e .[PROVIDER_EXTRA]`` will install the new dependencies. Tools like ``hatch`` can also
